...
import { CostingConfig, PrimaryProcessType, ProcessType } from '../costing.config';
import { PartComplexity } from 'src/app/shared/enums';
import { CastingConfigService } from 'src/app/shared/config/casting-config';
...
export class ManufacturingCastingCalculatorService {
  ...
  public doCostCalculationForCasting(manufactureInfo: ProcessInfoDto, fieldColorsList: any, manufacturingObj: ProcessInfoDto): ProcessInfoDto {
    ...
    const noOfCores = matCore?.coreCostDetails.length > 0 ? matCore?.coreCostDetails.reduce((accVal, currentRec) => accVal + currentRec.noOfCore, 0) || 1 : 0;
    if (IsHPDCCasting) {
    ...
    if (Number(manufactureInfo?.processTypeID) === ProcessType.CastingCorePreparation) {
      ...
      if (IsNoBakeCasting || IsGreenCasting || IsGDCCasting || IsLPDCCasting) {
        ...
        // }
        // // Slider Movement Length
        // if (!!manufactureInfo.platenSizeLength) {
        //   manufactureInfo.platenSizeLength = this.shareService.isValidNumber(Number(manufactureInfo.platenSizeLength));
        // } else {
        //   manufactureInfo.platenSizeLength = 0; // default
        // }
        // // Slider Movement Width
        // if (!!manufactureInfo.platenSizeWidth) {
        //   manufactureInfo.platenSizeWidth = this.shareService.isValidNumber(Number(manufactureInfo.platenSizeWidth));
        // } else {
        //   manufactureInfo.platenSizeWidth = 0; // default
        // }
        ...
        if (manufactureInfo.issandShootingDirty && !!manufactureInfo.sandShooting) {
          ...
          let sandShooting = (selectedMatcore[0]?.coreVolume * matMetal?.noOfCavities) / Math.pow(10, 6);
        ...
        if (IsGDCCasting || IsLPDCCasting || IsGreenCastingSemiAuto) {
          manufactureInfo.recommendTonnage = Math.ceil(((selectedMatcore[0]?.coreVolume * matMetal?.noOfCavities) / Math.pow(10, 6)) * matCore?.density) || 0;
        } else if (IsGreenCastingAuto) {
          manufactureInfo.recommendTonnage = Math.ceil((selectedMatcore[0]?.coreVolume * matMetal?.noOfCavities) / Math.pow(10, 6)) || 0;
        ...
        // if (!manufactureInfo.coreCycleTimes && matCore?.coreCostDetails?.length) {
        //   manufactureInfo.coreCycleTimes = Array.from({ length: matCore?.coreCostDetails?.length }).fill(0);
        // } else {
        //   manufactureInfo.coreCycleTimes.length = matCore?.coreCostDetails?.length || 0;
        // }
        // const manufObjCoreCycleTimes = manufacturingObj?.coreCycleTimes?.split(',');
        // matCore?.coreCostDetails.forEach((coreCost, i) => {
        //   if (!!manufactureInfo.isCoreCycleTimesDirty && !!manufactureInfo.isCoreCycleTimesDirty[i] && !!Number(manufactureInfo.coreCycleTimes[i])) {
        //     manufactureInfo.coreCycleTimes[i] = this.shareService.isValidNumber(Number(manufactureInfo.coreCycleTimes[i]));
        //   } else {
        //     const gassing = (coreCost?.coreWeight || 0) * 1000 * 0.0015 * 0.04;
        //     let coreCycleTimeItem = this.shareService.isValidNumber(dragCopeMovement + sandShooting + gassing + coreMovement + unloadingCore);
        //     if (Number(manufactureInfo.coreCycleTimes[i])) {
        //       coreCycleTimeItem = this.shareService.checkDirtyProperty('coreCycleTimes', fieldColorsList) ? manufObjCoreCycleTimes[i] : coreCycleTimeItem;
        //     }
        //     manufactureInfo.coreCycleTimes[i] = coreCycleTimeItem;
        //   }
        //   cycleTime += Number(manufactureInfo.coreCycleTimes[i]);
        // });
        ...
        //         .find((x) => x.fromWeight <= matCore?.totalCoreWeight && x.toWeight >= matCore?.totalCoreWeight && x.fromVolume <= matCore?.totalSandVolume && x.toVolume >= matCore?.totalSandVolume)
        //         ?.type || 1;
        //     manufactureInfo.subProcessTypeID = Number(manufactureInfo.subProcessTypeID) ? manufactureInfo.subProcessTypeID : machineType;

        //     const coreSandWeightMax =
        //       matCore?.coreCostDetails.length > 0 ? matCore?.coreCostDetails.reduce((selectedVal, currentRec) => (selectedVal > currentRec.coreWeight ? selectedVal : currentRec.coreWeight), 0) : 0;
        //     const coresPerHr = 15;
        //     const coreSandReqPerHr = coreSandWeightMax * coresPerHr;
        //     manufactureInfo.recommendTonnage = Math.ceil(coreSandReqPerHr / 1000); // Machine capacity required(tons/hour)

        //     const sandShootingSpeed = Number(manufactureInfo?.sandShooting) || 2000 / 60; //confirm machine db before removing default
        ...
        //         manufactureInfo.coreCycleTimes[i] = this.shareService.isValidNumber(Number(manufactureInfo.coreCycleTimes[i]));
        //       } else {
        //         const coreSandFillTime = sandShootingSpeed ? this.shareService.isValidNumber((coreCost?.coreWeight / sandShootingSpeed) * 60) : 0;
        //         const sandRamming = this.shareService.isValidNumber(coreCost?.coreArea / 4900);
        //         let coreCycleTimeItem = this.shareService.isValidNumber(coreSandFillTime + sandRamming + 20 + 60);
        //         if (Number(manufactureInfo.coreCycleTimes[i])) {
        //           coreCycleTimeItem = this.shareService.checkDirtyProperty('coreCycleTimes', fieldColorsList) ? manufObjCoreCycleTimes[i] : coreCycleTimeItem;
        //         }
        //         manufactureInfo.coreCycleTimes[i] = coreCycleTimeItem;
      ...
      if (IsGreenCasting) {
        ...
        // if (!manufactureInfo.subProcessTypeID && IsGreenCastingAuto) {
        //   manufactureInfo.subProcessTypeID = matMetal?.netWeight > 20 ? 2 : 1;
        // }
      ...
      // let dieSprayingDrying = (Number(manufactureInfo?.lengthOfCoated) * Number(manufactureInfo?.widthOfCoated)) / 55000;
      // dieSprayingDrying = dieSprayingDrying > 5 ? 5 : dieSprayingDrying;
      ...
      // const netWeightGrams = matMetal?.netWeight / 1000;
      const partRemoval = 0; // this._costingConfig.loadingUnloadingTime().find((x) => x.fromWeight <= netWeightGrams && x.toWeight >= netWeightGrams)?.time || 0;
      ...
      // const minThickness = matMetal?.partTickness || 0;
      ...
      // const projectedArea = manufactureInfo.partArea / Math.pow(10, 6);
      // const surfaceArea = manufactureInfo.flashArea / Math.pow(10, 6);
      const solidificationTime = solidificationTimeConst + maxThickness * coolingFactor * (surfaceArea / projectedArea);
      // const solidificationTime = solidificationTimeConst + minThickness * coolingFactor * ((surfaceArea / Math.pow(10, 6) / projectedArea) * 0.5) - minThickness;
      ...
      // const partRemoval = this._costingConfig.loadingUnloadingTime().find((x) => x.fromWeight <= matMetal?.netWeight && x.toWeight >= matMetal?.netWeight)?.time || 0;
