...
import { DfMachineInfoDto } from '../../digital-factory/Models/df-machine-info-dto';
import { DfMaterialInfoDto } from '../../digital-factory/Models/df-material-info-dto';
...
export class DigitalFactoryHelper {
  ...
  getMaterialInfo(
    ...
    let matPrice = getMatValues(() => dfMatInfo?.price, stockFormMultiplier ? materialMarket?.price * stockFormMultiplier : materialInfo?.materialPricePerKg || materialMarket?.price);
    ...
    let dfStockFormName = stockFormList?.find((x) => x.stockFormId === dfMatInfo?.stockFormId)?.formName;
    let stockForm = dfStockFormName ?? stockFormName;
    if (this.sharedService.checkDirtyProperty('matPrice', fieldColorsList) && materialInfo) {
      matPrice = materialInfo?.materialPricePerKg;
    }
    if (this.sharedService.checkDirtyProperty('volumePurchased', fieldColorsList) && materialInfo) {
      volumePurchased = materialInfo?.volumePurchased;
    }
    if (this.sharedService.checkDirtyProperty('scrapPrice', fieldColorsList) && materialInfo) {
      scrapPrice = materialInfo?.scrapPricePerKg;
    }
    if (this.sharedService.checkDirtyProperty('stockForm', fieldColorsList) && materialInfo) {
      stockForm = materialInfo?.stockForm;
    }
    if (materialMaster) {
      volumeDiscount = getMatValues(() => this.transformDiscountPercentageToValue(dfMatInfo?.discountPercent), this.getVolumeDiscount(Number(volumePurchased), materialMaster));
    }
    ...
    if (this.sharedService.checkDirtyProperty('volumeDiscountPer', fieldColorsList) && materialInfo) {
      volumeDiscount = materialInfo?.volumeDiscountPer;
    }
  ...
  calculateAndGetMhrValue(
    ...
    let modifiedMhr;
    if (this.sharedService.checkDirtyProperty('machineHourRate', fieldColorsList)) {
    ...
    const anualHours = this.getAnualHours(this.supplierInfo.supplierDirectoryMasterDto) || countryData?.annualHours;
  ...
  setDigitalFactoryInfo(supplierId: number) {
    if (!supplierId) return;
    this.digitalFactoryService
      .getDigitalFactoryBySupplierId(supplierId)
      .pipe(take(1))
      .subscribe({
        next: (data) => {
  ...
  private getDFTotalPowerCost() {
    ...
    const costValues = [
      supplierMaster?.coalCost * supplierMaster?.coalPortion,
      supplierMaster?.windCost * supplierMaster?.windPortion,
      supplierMaster?.naturalGasCost * supplierMaster?.naturalGasPortion,
      supplierMaster?.nuclearCost * supplierMaster?.nuclearPortion,
      supplierMaster?.geothermalCost * supplierMaster?.geothermalPortion,
      supplierMaster?.otherNonRenewableCost * supplierMaster?.otherNonRenewablePortion,
      supplierMaster?.otherRenewableCost * supplierMaster?.otherRenewablePortion,
    ].filter((x) => x !== 0 && x !== null);

    return this.sharedService.isValidNumber(costValues.reduce((acc, num) => acc + num, 0) / this.getDFTotalPowerSupply());
  }
