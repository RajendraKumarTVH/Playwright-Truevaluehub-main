import { Injectable, effect, OnDestroy } from '@angular/core';
...
import { BlockUiService, MedbMasterService } from 'src/app/shared/services';
...
import { SheetMetalProcessCalculatorService } from '../manufacturing-sheetmetal-calculator.service';
...
import { WeldingCalculatorService } from '../manufacturing-welding-calculator.service';
import { CostingCompletionPercentageCalculator } from '../costing-completion-percentage-calculator';
import { CostToolingRecalculationService } from './cost-tooling-recalculation.service';
import { PlasticRubberProcessCalculatorService } from '../plastic-rubber-process-calculator.service';
import { ManufacturingForgingCalculatorService } from '../manufacturing-forging-calculator.service';
...
import { ManufacturingWireCuttingTerminationCalculatorService } from '../manufacturing-wire-cutting-termination-calculator.service';
import { CustomCableService } from '../manufacturing-custom-cable.service';
import { SecondaryProcessCalculatorService } from '../manufacturing-secondary-process.service';
...
export class CostManufacturingAutomationService implements OnDestroy {
  ...
  // _bulkProcessUpdateLoading$ = this._store.select(ProcessInfoState.getBulkProcessUpdateStatus);
  // _updatedProcessInfoList$ = this._store.select(ProcessInfoState.getProcessInfos);
  private automationResultEvent = new Subject<any>();
  automationResult$ = this.automationResultEvent.asObservable();
  bulkUpdateProcessSubscription$: Subscription = Subscription.EMPTY;
  selectedMachineIndex = 0;
  private recalcContext: any = {};
  newCoreAdded: boolean = false;
  private processRecalcEffect = effect(() => {
    const bulkProcessUpdateLoading = this.recalculationUpdateSignalsService.bulkProcessUpdateLoading();
    const updatedProcessInfoList = this.processInfoSignalService.processInfos();
    if (!bulkProcessUpdateLoading && updatedProcessInfoList.length >= this.recalcContext.calculateResults?.length) {
      this.handleProcessRecalc(updatedProcessInfoList);
    } else if (!this.recalcContext.calculateResults && !bulkProcessUpdateLoading) {
  ...
  constructor(
    ...
    private manufacturingMachiningConfigService: ManufacturingMachiningConfigService,
    ...
    private _manufacturingForgingSubProcessConfig: ManufacturingForgingSubProcessConfigService,
  ...
  saveRecalculateResult(
  ...
  ): { selectedProcessInfoId: number; formIdentifier: CommentFieldFormIdentifierModel } {
    ...
    if (automationProcessCount > 0 && automationProcessCount === calculateResults.length) {
      ...
      // bulkUpdateProcessSubscription$ = this._bulkProcessUpdateLoading$.subscribe((bulkProcessUpdateLoading) => {
      //   if (!bulkProcessUpdateLoading) {
      //     bulkUpdateProcessSubscription$.unsubscribe();
      //     this._updatedProcessInfoList$.subscribe((updatedProcessInfoList) => {
      //       const processMapping = this._manufacturingConfig.processMappingSortCastingAutomation;
      //       for (const process in processMapping) {
      //         for(const materialInfo of materialInfoList) {
      ...
      //             updatedProcessInfoList = [...this.sharedService.sortObjectbyInteger(updatedProcessInfoList, 'processTypeID', processMapping[process])]
      //           }
      //         }
      //       }
      //       !selectedProcessInfoId && (selectedProcessInfoId = updatedProcessInfoList[updatedProcessInfoList.length - 1]?.processInfoId || 0);
      //       formIdentifier = { ...formIdentifier, primaryId: selectedProcessInfoId };
      //     });
      // this.bulkUpdateProcessSubscription$ = combineLatest([this._bulkProcessUpdateLoading$, this._updatedProcessInfoList$])
      //   .pipe(
      //     filter(([bulkProcessUpdateLoading, updatedProcessInfoList]) => bulkProcessUpdateLoading === false && updatedProcessInfoList.length >= calculateResults.length),
      //     take(1)
      //   )
      //   .subscribe(([_, updatedProcessInfoList]) => {
      // if (bulkProcessUpdateLoading === false && updatedProcessInfoList.length >= calculateResults.length) {
      // this.bulkUpdateProcessSubscription$.unsubscribe();
      // this._store.dispatch(new ProcessInfoActions.SetBulkProcessUpdateLoading(true));
      //   this.processInfoSignalService.setBulkProcessUpdateLoading(true);
      //   const processMapping = this._manufacturingConfig.processMappingSortCastingAutomation;
      ...
      //     }
      //   }
      //   if (!selectedProcessInfoId) {
      //     selectedProcessInfoId = updatedList[0]?.processInfoId || 0;
      //   }
      //   formIdentifier = {
      //     ...formIdentifier,
      //     primaryId: selectedProcessInfoId,
      //   };
      //   this.recalculationCompletedEvent.next({ materialInfoList, calculateResults, currentPart, isToolingNeedToRun });
      //   this.automationResultEvent.next({ selectedProcessInfoId, formIdentifier, inputMachineTypeDescription: this.machineTypeDescription });
      //   this.blockUiService.popBlockUI('recalculate ProcessCost');
      //   this.automationProcessCount = 0;
      //   this.messaging.openSnackBar(`Recalculation completed for Manufacturing Section.`, '', { duration: 5000 });
      //   // }
      // });
      // !selectedProcessInfoId && (selectedProcessInfoId = calculateResults[0]?.processInfoId || 0);
      // formIdentifier = { ...formIdentifier, primaryId: selectedProcessInfoId };
    }
    return { selectedProcessInfoId, formIdentifier };
  }
  ...
  private handleProcessRecalc(updatedProcessInfoList: ProcessInfoDto[]) {
    ...
    this.recalculationCompletedEvent.next({
      materialInfoList: this.recalcContext.materialInfoList,
      calculateResults: this.recalcContext.calculateResults,
      currentPart: this.recalcContext.currentPart,
      isToolingNeedToRun: this.recalcContext.isToolingNeedToRun,
    });
    this.automationResultEvent.next({
      selectedProcessInfoId: this.recalcContext.selectedProcessInfoId,
      formIdentifier: this.recalcContext.formIdentifier,
  ...
  selectMachineProcess(
    ...
    if (PrimaryProcessType.HPDCCasting === materialInfo?.processId) {
      ...
      netWeight = matMaterial.netWeight / 1000; // converting to kg
    }
    ...
    if (currentPart?.commodityId == CommodityType.PlasticAndRubber) {
      if ([ProcessType.InjectionMouldingSingleShot, ProcessType.InjectionMouldingDoubleShot, ProcessType.PlugConnectorOvermolding].includes(processInfo.processTypeID)) {
        ...
        processInfo.recommendTonnage = this.sharedService.isValidNumber(recommendTonnage);
        ...
        // processInfo.shotSize = manufactureInfo.machineMaster?.shotSize || 0;
        // processInfo.recBedSize = (processInfo.platenSizeLength && processInfo.platenSizeWidth) ? (processInfo.platenSizeLength +' x '+ processInfo.platenSizeWidth) : '';
        // processInfo.selectedBedSize = (processInfo.machineMaster?.platenLengthmm || '0') + ' x '+ (processInfo.machineMaster?.platenWidthmm || '0');
      ...
      //   const internalCavityPressure = 2.5; // default
      //   const recommendTonnage = (1.15 * (matMaterial?.noOfCavities || 1) * internalCavityPressure * matMaterial?.partProjectedArea) / 1000;
      //   processInfo.recommendTonnage = this.sharedService.isValidNumber(recommendTonnage);
      //   if (processInfo.recommendTonnage > 0) {
      //     machine = machineTypeDescription.filter((x) => x.machineTonnageTons > recommendTonnage).sort((a, b) => a.machineTonnageTons - b.machineTonnageTons);
      //   } else {
      ...
      if ([ProcessType.LaserCutting, ProcessType.PlasmaCutting].includes(processInfo.processTypeID)) {
        ...
        if (ProcessType.PlasmaCutting === processInfo.processTypeID) {
          ...
          //     (x) =>
          //       // x.machineMarketDtos.find((y) => y.machineMarketID == sortedMachineList[0]?.machineMarketId) &&
          //       x.bedLength >= materialInfo?.coilLength &&
          //       // x.bedWidth >= materialInfo?.coilWidth &&
          //       // x.plasmaCurrent >= processInfo?.totalFactorySpaceRequired
          //       (x.plasmaCurrent >= minAmps && x.plasmaCurrent <= maxAmps)
          //   );
          //   if (plasmaFilterdMachine.length > 0) {
          //     machineTypeDescription = plasmaFilterdMachine;
          //   } else {
          //     machineTypeDescription = machineTypeDescription.filter(x => x.plasmaCurrent >= minAmps && x.bedLength >= materialInfo?.coilLength);
          //   }
          // }
        } else {
        ...
        // if (thickness <= 2) {
        //   processInfo.platenSizeLength = 427;
        // } else if (thickness <= 4) {
        //   processInfo.platenSizeLength = 447;
        // } else if (thickness <= 5) {
        //   processInfo.platenSizeLength = 487;
        // }
      ...
      if (processInfo.processTypeID === ProcessType.CastingCorePreparation) {
      ...
      } else if ([ProcessType.MoldPerparation, ProcessType.CastingMoldMaking].includes(processInfo.processTypeID)) {
        ...
        if (PrimaryProcessType.NoBakeCasting === matMaterial?.processId) {
          this.selectedMachineIndex = machine.findIndex(
            (x) => x.machineDescription.toLowerCase().includes('manual molding line') && x.flaskLength >= mouldSandMaterial?.moldBoxLength && x.flaskWidth >= mouldSandMaterial?.moldBoxWidth
          );
          ...
          this.selectedMachineIndex = machine.findIndex((x) => x.flaskLength >= mouldSandMaterial?.moldBoxLength && x.flaskWidth >= mouldSandMaterial?.moldBoxWidth);
        }
        ...
        this.selectedMachineIndex = machine.findIndex((x) => x.machineTonnageTons >= processInfo.recommendTonnage && x.machineName.toLowerCase().includes(searchTerm));
        if (this.selectedMachineIndex === -1) {
        ...
        this.selectedMachineIndex = machine.findIndex((x) => x.pourCapacity >= processInfo.recommendTonnage);
        ...
        machine = machineTypeDescription
          .filter((x) => x.machineTonnageTons > processInfo.recommendTonnage && x.bedLength > dieSetLength && x.bedWidth > dieSetWidth)
          .sort((a, b) => a.machineTonnageTons - b.machineTonnageTons);
        ...
        this.selectedMachineIndex = machine.findIndex((x) => x.platenLengthmm > matMaterial.dimX && x.platenWidthmm > matMaterial.dimY);
        ...
        // } else if (processInfo.processTypeID === ProcessType.CastingCoreAssembly) {
        //   const coreSandMaterial = processInfo.materialInfoList?.find((x) => x?.secondaryProcessId === 2);
        //   const maxCoreDim = coreSandMaterial.coreCostDetails.reduce(
        //     (acc: any, curr) => (curr.coreLength >= acc.length && curr.coreWidth >= acc.width ? { length: curr.coreLength, width: curr.coreWidth } : acc),
        //     {
        ...
        //     }
        //   );
        //   machine = machineTypeDescription.sort((a, b) => a.flaskLength * a.flaskWidth - b.flaskLength * b.flaskWidth);
        //   this.selectedMachineIndex = machine.findIndex((x) => x.flaskLength >= maxCoreDim.length && x.flaskWidth >= maxCoreDim.width);
        ...
        if (matMaterial?.processId === PrimaryProcessType.NoBakeCasting) {
          ...
          this.selectedMachineIndex = machine.findIndex((x) => x.maxProcessableWeightKgs >= netWeight && x.machineMarketDtos.find((y) => y.machineType === 'Automatic'));
        } else {
      ...
      // } else {
      // machine = machineTypeDescription.sort((a, b) => (a.machineTonnageTons < b.machineTonnageTons ? -1 : 1));
      // } // Enable this only after the machine db is updated with correct machineTonnageTons
      ...
      if (processInfo.processTypeID === ProcessType.SawCutting) {
        ...
        // machine = machineTypeDescription.sort((a, b) => (a.workPieceMinOrMaxDia < b.workPieceMinOrMaxDia ? -1 : 1));
        // this.selectedMachineIndex = machine.findIndex((x) => x.workPieceMinOrMaxDia >= processInfo.recommendTonnage);
        ...
        const targetMachineName = machineNameMap[processInfo?.subProcessTypeID];
      ...
      this.digitalFactoryService
        ...
        .subscribe((machineTypeDescription: MedbMachinesMasterDto[]) => {
          if (machineTypeDescription && machineTypeDescription.length > 0) {
            ...
            // const moldBoxSize = this.setMoldPreparationData(machineInfoList)?.moldBoxSize || 0;
              ...
              if (machineTypeObj && processInfo?.machineMarketId) {
                ...
                // }
                  ...
                  if (calculationRes) {
                    ...
                    if (calculationRes.processTypeID === ProcessType.HighPressureDieCasting) {
                      ...
                      if (this.newCoreAdded && this.automationProcessCount > 0 && this.automationProcessCount === this.totProcessList.length) {
                        ...
                        const selectedProcess = this.saveRecalculateResult(
                          this.totProcessList,
                          materialInfoList,
                          currentPart,
                          this.automationProcessCount,
                  ...
                  if (materialInfo?.processId == PrimaryProcessType.InjectionMouldingSingleShot || materialInfo?.processId == PrimaryProcessType.InjectionMouldingDoubleShot) {
                    ...
                    this.handleRecalculationResult(
                      this._plasticRubberService.calculationsForInjectionMoulding(processInfo, fieldColorsList, manufacturingObj),
                      materialInfoList,
                      currentPart,
                      toolingEntryData,
                      manufacturingObj,
                    ...
                    // const toolingEntryData = { toolNameId: InjectionMouldingTool.InjectionMoulding, materialInfo: materialInfo, processInfo: processInfo };
                    this.handleRecalculationResult(
                      this._plasticRubberService.calculationsForRubberInjectionMoulding(processInfo, fieldColorsList, manufacturingObj),
                      materialInfoList,
                      currentPart,
                      null,
                      manufacturingObj,
                    ...
                    this.handleRecalculationResult(
                      this._plasticRubberService.calculationsForRubberExtrusion(processInfo, fieldColorsList, manufacturingObj),
                      materialInfoList,
                      currentPart,
                      null,
                      manufacturingObj,
                    ...
                    processInfo.newToolingRequired = false;
                    this.handleRecalculationResult(
                      this._plasticRubberService.calculationsForCompressionMolding(processInfo, fieldColorsList, manufacturingObj),
                      materialInfoList,
                      currentPart,
                      null,
                      manufacturingObj,
                    ...
                    if (processInfo?.processTypeID === ProcessType.Cutting) {
                      ...
                      this.handleRecalculationResult(
                        this._plasticRubberService.calculationsForTransferMolding(processInfo, fieldColorsList, manufacturingObj),
                        materialInfoList,
                        currentPart,
                        null,
                        manufacturingObj,
                    ...
                    if (processInfo?.processTypeID === ProcessType.Cutting) {
                      ...
                      this.handleRecalculationResult(
                        this._plasticRubberService.doCostCalculationForVacuumForming(processInfo, fieldColorsList, manufacturingObj),
                        materialInfoList,
                        currentPart,
                        toolingEntryData,
                        manufacturingObj,
                    ...
                    this.handleRecalculationResult(
                      this._plasticRubberService.calculationsForBlowMolding(processInfo, fieldColorsList, manufacturingObj, currentPart),
                      materialInfoList,
                      currentPart,
                      null,
                      manufacturingObj,
  ...
  ngOnDestroy(): void {
    this.processRecalcEffect.destroy();
  }
}
