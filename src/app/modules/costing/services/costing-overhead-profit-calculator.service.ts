...
import { FormDataModel } from '../../../shared/models/simulation/costingOverHeadProfit-Simulation.model';
...
export class CostingOverheadProfitCalculatorService {
  ...
  calculateOverheadCost(
    ...
    //const costingOverHeadProfit: CostOverHeadProfitDto = { ...costingOverHeadProf };
  ...
  getAndSetData(costSummaryViewData: ViewCostSummaryDto, annualVolume: number, lotSize: number, paymentTermId: number, getCostingOverHeadProfit: CostOverHeadProfitDto, commodityId: number) {
    ...
    // costingOverHeadProfit.ICCAmount = 0;//TODO:Calculations not provided
    const eXWPartCostAmount =
      Number(materialCost) +
      Number(costSummaryViewData?.sumNetProcessCost) +
      //Number(costSummaryViewData.sumOverHeadCost) +
    ...
    // costingOverHeadProfit.FinishPartICCAmount = 0;//TODO:Calculations not provided
    const paymentDays = this.paymentTermsMaster();

    const paymentTerms = (((getCostingOverHeadProfit.paymentTermsPer / 100) * eXWPartCostAmount) / 365) * (paymentDays.get(paymentTermId) - 30) || 0;
  ...
  setFormData(costOverHeadProfitobj: CostOverHeadProfitDto) {
    ...
    // formDataModel.total = (costOverHeadProfitobj?.mohCost || 0) + (costOverHeadProfitobj?.fohCost || 0) + (costOverHeadProfitobj?.sgaCost || 0);
    formDataModel.total =
