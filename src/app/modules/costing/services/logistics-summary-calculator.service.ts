...
import { MaterialInfoDto, PartInfoDto, BuLocationDto } from '../../../shared/models';
import { LogisticsSummaryService } from 'src/app/shared/services/logistics-summary.service';
import { FreightCostCalcResponseDto } from 'src/app/shared/models/freight-cost-response';
import { ManualFreightCostRequestDto } from 'src/app/shared/models/freight-cost-request';
import { ContainerTypeEnum, LogisticsSummaryDto, ModeOfTransportEnum, ShipmentTypeEnum } from 'src/app/shared/models/logistics-summary.model';
...
export class LogisticsSummaryCalculatorService {
  ...
  public getPercentageOfContainerRequired(
    ...
    if (materialList) {
      for (let i = 0; i < materialList.length; i++) {
        ...
        // if (material?.dimX && material?.dimY && material?.dimZ) {
        //   totalShipmentVolume += (partsPerShipment * material?.dimX * material?.dimY * material?.dimZ) / 1000;
        // }
      }
    }
  ...
  // public shipmentCost(fullContainerCost: number, containerPercent: number, dirtyFieldList: FieldColorsDto[]) {
  //   const shipmentCost = fullContainerCost * (containerPercent / 100);
  //   return shipmentCost;
  // }
  ...
  getCostCalculation(
    ...
    // //Now the freight cost is calculated per shipment not by annually,
    // // Later if we want to show the annual cost uncomment this part.
    dto.annualShipment = 1; //this.part?.deliveryFrequency ?? 0;

    //HarCode this value to EXW for now to get the Door to Door Cost
    // Later this will change to get from part
    dto.part = part;
    dto.incoTerm = 'EXW'; //this.part.incoTerm;
    ...
    return this.logisticsSummaryService.getOfflineFreightCost(dto).pipe(
      map((result: any) => {
        if (result) {
          ...
          // freightCost.containerCost = this.sharedService.isValidNumber(result.totalAnnualCost);
  ...
  calculateLogisticsCost(logistic: LogisticsSummaryDto, dirtyList: FieldColorsDto[], summaryObj: LogisticsSummaryDto): Observable<LogisticsSummaryDto> {
    if (logistic?.isContainerPercentDirty && logistic?.containerPercent != null) {
      ...
      // let containerPercent = logistic.containerPercent;
    ...
    if (logistic?.isContainerCostDirty && logistic?.containerCost != null) {
      ...
      // let containerCost = logistic?.containerCost;
    ...
    if (logistic?.isFreightCostPerShipmentDirty && logistic?.freightCostPerShipment != null) {
      ...
      // let freightCostPerShipment = this.sharedService.isValidNumber(Number(logistic.containerCost) * (Number(logistic.containerPercent) / 100));
    ...
    if (logistic?.packagingInfo?.partsPerShipment) {
      ...
      // partsPerShipment = this.sharedService.isValidNumber((logistic?.currentPart?.eav / Number(logistic?.currentPart?.deliveryFrequency)) * 365);
    ...
    if (logistic?.isFreightCostDirty && logistic?.freightCost != null) {
      ...
      // let freightCost = this.sharedService.isValidNumber(logistic.freightCostPerShipment / partsPerShipment);
      let freightCost =
    ...
    //     totalCarbonFootPrint = this.checkDirtyProperty('TotalCarbonFootPrint', sustainabilityDirtyFields) ? summaryObj?.totalCarbonFootPrint : totalCarbonFootPrint;
    //   }
    ...
    //     carbonFootPrint = this.checkDirtyProperty('CarbonFootPrint', sustainabilityDirtyFields) ? summaryObj?.carbonFootPrint : carbonFootPrint;
    //   }
    ...
    //     carbonFootPrintPerUnit = this.checkDirtyProperty('CarbonFootPrintPerUnit', sustainabilityDirtyFields) ? summaryObj?.carbonFootPrintPerUnit : carbonFootPrintPerUnit;
    //   }
