...
import { pcbReportName } from 'src/app/shared/constants';
...
@Component({
  selector: 'app-pcb-report-component',
  templateUrl: './pcb-report-component.html',
  styleUrls: ['./pcb-report-component.scss'],
  standalone: true,
  imports: [CommonModule, FormsModule, ReactiveFormsModule],
})
export class PcbReportComponent implements OnInit, OnDestroy {
  public screenHeight: number;
  private unsubscribe$: Subject<undefined> = new Subject<undefined>();
  private pcbReport?: ReportModel;
    ...
    this.overviewService
      ...
      .subscribe((result: EmbedParametersDto) => {
        if (result?.embedToken) {
          this.pcbReport = this.config.configuration.reports?.find((r) => r.reportName === pcbReportName);
  ...
  showReport(accessToken: any) {
    // Embed URL
    if (!this.pcbReport) return;
    const embedUrl = this.pcbReport.reportUrl;
    const embedReportId = this.pcbReport.reportIdentifier;
    const config = {
      type: 'report',
