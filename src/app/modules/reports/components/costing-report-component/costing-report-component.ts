...
import { costingReportName } from 'src/app/shared/constants';
import { ReportModel } from 'src/app/modules/settings/models';
import { CommonModule } from '@angular/common';
import { FormsModule, ReactiveFormsModule } from '@angular/forms';

@Component({
  selector: 'app-costing-report-component',
  templateUrl: './costing-report-component.html',
  styleUrls: ['./costing-report-component.scss'],
  standalone: true,
  imports: [CommonModule, FormsModule, ReactiveFormsModule],
})
export class CostingReportComponent implements OnInit, OnDestroy {
  public screenHeight: number;
  private unsubscribe$: Subject<undefined> = new Subject<undefined>();
  private costingReport?: ReportModel;
    ...
    this.overviewService
      ...
      .subscribe((result: EmbedParametersDto) => {
        if (result?.embedToken) {
          this.costingReport = this.config.configuration.reports?.find((r) => r.reportName === costingReportName);
  ...
  showReport(accessToken: any) {
    // Embed URL
    if (!this.costingReport) return;
    const embedUrl = this.costingReport.reportUrl;
    const embedReportId = this.costingReport.reportIdentifier;
    const config = {
      type: 'report',
