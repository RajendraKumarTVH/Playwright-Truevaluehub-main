...
export class PCBRPAConfigService {
  ...
  getDiscountBasedOnSubcategoryForMpn(info: PCBAMarketDataDto, vol: number) {
    ...
    const discountValue = this.sharedService.isValidNumber((info?.price * discountPerc) / 100);
    const valueAfterDisc = this.sharedService.isValidNumber(info.price - discountValue);
