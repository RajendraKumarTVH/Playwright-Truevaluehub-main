...
(function (factory) {
  ...
  $.jstree.plugins.node_customize = function (options, parent) {
    this.redraw_node = function (obj) {
      const el = parent.redraw_node.apply(this, arguments);
      if (el) {
        const node = this.get_node(obj);
        const cfg = this.settings.node_customize;
        const key = cfg.key;
        const type =  (node && node.original && node.original[key]);
        const customizer = (type && cfg.switch[type]) || cfg.default;
        if(customizer)
        {customizer(el, node);}
      }
      return el;
