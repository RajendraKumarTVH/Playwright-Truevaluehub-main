...
import { MaterialCastingMappingService } from 'src/app/shared/mapping/material-casting-mapping.service';
...
export class MaterialCastingCalculatorService implements IMaterialCalculationByCommodity {
  ...
  public calculationsForPouringCasting(materialInfo: MaterialInfoDto, fieldColorsList: any, selectedMaterialInfo: MaterialInfoDto, currentPart: PartInfoDto): MaterialInfoDto {
    ...
    if (materialInfo.isPartVolumeDirty && !!materialInfo.partVolume) {
      ...
      // let partVolume = this.shareService.isValidNumber(Number(materialInfo.dimX) * Number(materialInfo.dimY) * Number(materialInfo.dimZ));
    ...
    if (materialInfo.isPartSurfaceAreaDirty && !!materialInfo.partSurfaceArea) {
      ...
      //     : Number(materialInfo.processId) === PrimaryProcessType.LPDCCasting
      //       ? this.shareService.isValidNumber(Number(materialInfo.partVolume) / Number(materialInfo.dimZ))
      //       : Number(materialInfo.partSurfaceArea);
      ...
      // } else {
      //   materialInfo.partSurfaceArea = materialInfo.partSurfaceArea;
      // }
    }
    ...
    // Below are doing in the manufacturing casting calculation service
    // const runnerProjectedArea = 60 / 100;
    // const totalProjectedArea = (Number(materialInfo.partProjectedArea) + (Number(materialInfo.partProjectedArea) * runnerProjectedArea)) * materialInfo.noOfCavities;

    // if ([PrimaryProcessType.HPDCCasting, PrimaryProcessType.GDCCasting, PrimaryProcessType.LPDCCasting].includes(materialInfo.processId)) {
    //   materialInfo.runnerRiserPercentage = this.materialCastingConfigService.getRunnerRaiserPercentageByThickness(materialInfo.partTickness, currentPart?.partComplexity, materialInfo.processId);
    //   materialInfo.oxidationLossWeightPercentage = this.materialCastingConfigService.getIrretrivalLossPercentage(materialInfo.materialFamily);
    // } else {
    if (materialInfo.isRunnerRiserPercentageDirty && !!materialInfo.runnerRiserPercentage) {
      ...
      const runnerRiserPercentage = [PrimaryProcessType.HPDCCasting, PrimaryProcessType.GDCCasting, PrimaryProcessType.LPDCCasting].includes(materialInfo.processId)
        ? this.materialCastingConfigService.getRunnerRaiserPercentageByThickness(materialInfo.partTickness, currentPart?.partComplexity, materialInfo.processId)
        : this.materialCastingConfigService.getRunnerRaiserPercentageByWeight(materialInfo.materialFamily, materialInfo.netWeight, currentPart?.partComplexity) ||
      ...
      const runnerRiser =
        materialInfo.processId === PrimaryProcessType.InvestmentCasting
          ? // ? this.materialCastingConfigService.sprueGateWeightCalculation(Number(materialInfo.density)).totMetalWeightkg / this.castingConfigService.castingConstants.finalCavitiesPerTree
            this.shareService.isValidNumber(this.materialCastingConfigService.sprueGateWeightCalculation(Number(materialInfo.density)).totMetalWeightkg / materialInfo.noOfCavities)
          : this.shareService.isValidNumber(Number(materialInfo.netWeight) * (Number(materialInfo.runnerRiserPercentage) / 100));
    ...
    if (materialInfo.isSandRecoveryDirty && !!materialInfo.sandRecovery) {
      ...
      if (materialInfo.sandRecovery) {
        sandRecovery = this.shareService.checkDirtyProperty('sandRecovery', fieldColorsList) ? selectedMaterialInfo?.sandRecovery : sandRecovery;
      }
    ...
    // const materialUnderRecurculation =
    //   Number(materialInfo.processId) === PrimaryProcessType.GDCCasting
    //     ? Number(materialInfo.runnerRiser)
    //     : Number(materialInfo.runnerRiser) - Number(materialInfo.sandRecovery);
