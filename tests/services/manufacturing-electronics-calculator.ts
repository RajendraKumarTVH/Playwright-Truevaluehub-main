...
export class ManufacturingElectronicsCalculatorService {
  ...
  public calculationForElectronics(manufactureInfo: ProcessInfoDto, fieldColorsList: any, manufacturingObj: ProcessInfoDto): ProcessInfoDto {
    ...
    const pcbLength: number = Number(materialInfo?.sheetLength) || 0;
    const pcbWidth: number = Number(materialInfo?.sheetWidth) || 0;
    // const panelLength: number = Number(materialInfo?.sheetThickness);
    const panelWidth: number = Number(materialInfo?.inputBilletWidth) || 0;
    const noOfPCBs: number = Number(materialInfo?.totalCableLength) || 1;
    const noOfComponents: number = Number(materialInfo?.flashVolume) || 0;
    const application: number = Number(materialInfo?.noOfCables) || 0;
    const mountingSides: number = Number(materialInfo?.typeOfConductor) || 0;
    ...
    if (processId === ProcessType.MaterialKitting) {
      ...
      for (let i = 0; i < manufactureInfo.subProcessFormArray?.controls?.length; i++) {
        ...
        const noOfRadialTHComponentsPCB = Number(materialInfo?.mainInsulatorOD) || 0;
        const noOfAxialTHComponentsPCB = Number(materialInfo?.mainCableSheathingMaterial) || 0;
        const noOfSpecialTHComponentsPCB = Number(materialInfo?.pickPlaceTime) || 0;
        const noOfSpecialTHComponentsPinCount = Number(materialInfo?.openingTime) || 0;
        const noOfPressfitComponents = Number(materialInfo?.coilWidth) || 0;
      ...
      const ProgrammaleIcsPerPCB = materialInfo?.moldBoxLength || 0;
      ...
      for (let i = 0; i < manufactureInfo.subProcessFormArray?.controls?.length; i++) {
        ...
        const percentOfConformalCoating: number = Number(materialInfo?.partsPerCoil) || 0;
        const coatingSides: number = Number(materialInfo?.coilLength) || 0;
      ...
      for (let i = 0; i < manufactureInfo.subProcessFormArray?.controls?.length; i++) {
        const info = manufactureInfo.subProcessFormArray?.controls[i];
        const subProcessId: number = Number(info?.value?.subProcessTypeID);
        const coatingSides: number = Number(materialInfo?.coilWeight) || 0;
        const percentOfCoatingArea: number = Number(materialInfo?.partOuterDiameter) || 0;
      ...
      for (let i = 0; i < manufactureInfo.subProcessFormArray?.controls?.length; i++) {
        const info = manufactureInfo.subProcessFormArray?.controls[i];
        const subProcessId: number = Number(info?.value?.subProcessTypeID);
        const processType: number = Number(materialInfo?.ejectionTime) || 0;
      ...
      for (let i = 0; i < manufactureInfo.subProcessFormArray?.controls?.length; i++) {
        const info = manufactureInfo?.subProcessFormArray?.controls[i];
        const subProcessId: number = Number(info?.value?.subProcessTypeID);
        const lessThan6Pins: number = Number(materialInfo?.partInnerDiameter) || 0;
        const moreThan6AndLessThan14Pins: number = Number(materialInfo?.runnerRiser) || 0;
        const moreThan14Pins: number = Number(materialInfo?.oxidationLossWeight) || 0;
        const lessThan6PinsOnBotSide: number = Number(materialInfo?.pouringWeight) || 0;
        const moreThan6AndLessThan14PinsOnBotSide: number = Number(materialInfo?.cavityArrangementLength) || 0;
        const moreThan14PinsOnBotSide: number = Number(materialInfo?.cavityArrangementWidth) || 0;
        // const ProgrammaleIcsPerPCB: number = Number(materialInfo?.moldBoxLength);
        const NoofComplexComponents: number = Number(materialInfo?.moldBoxHeight) || 0;
