...
import {
  ...
  Soldermask,
  SurfaceFinish,
} from 'src/app/shared/config/manufacturing-pcb-config';
...
export class ConventionalPCBCalculatorService {
  constructor(
  ...
  public doCostCalculationForConventionalPCB(manufactureInfo: ProcessInfoDto, fieldColorsList: any, manufacturingObj: ProcessInfoDto) {
    let totalCycleTime = 0;
    const materialInfo = manufactureInfo?.materialInfoList?.length > 0 ? manufactureInfo?.materialInfoList[0] : null;
    const noOfPCB = materialInfo?.coilWeight * materialInfo?.txtWindows;
    ...
    for (let i = 0; i < manufactureInfo.subProcessFormArray?.controls?.length; i++) {
      ...
      if (Number(manufactureInfo?.processTypeID) === ProcessType.InnerLayer) {
        ...
        if (Number(info.value.subProcessTypeID) === PCBDrilling.XRayDrilling) {
          ...
          const NoofArrayperpanel = Number(materialInfo.coilWeight);
        ...
        if (Number(info.value.subProcessTypeID) === Soldermask.PreTreatment) {
          ...
          cycleTime = (((shortestSide + 0.1) / 3) * 60) / noOfPCB;
        } else if (Number(info.value.subProcessTypeID) === Soldermask.MaskCuring) {
        ...
        if (Number(info.value.subProcessTypeID) === RoutingScoring.Routing) {
          ...
          const CuttinglengthperPanel =
            (materialInfo.closingTime + materialInfo.injectionTime) * 2 * 2 * materialInfo.coilWeight * 2 + (((materialInfo.openingTime + materialInfo.colorantPer) * 2 * 80) / 100) * noOfPCB;
          ...
          const NoofArraysperpanel = materialInfo.coilWeight;
        ...
        const ipcClass = Number(materialInfo.primerDensity);
        if (Number(info.value.subProcessTypeID) === ETestBBT.BBT) {
          cycleTime = ipcClass === 3 ? (materialInfo.stockCrossSectionHeight / 6) * 0.25 : (materialInfo.stockCrossSectionHeight / 4) * 0.1;
        }
      }
