...
export class SheetMetalProcessCalculatorService {
  ...
  public calculationForBending(manufactureInfo: ProcessInfoDto, fieldColorsList: any, manufacturingObj: ProcessInfoDto): ProcessInfoDto {
    ...
    const theoreticalForceForce = (Math.pow(dimz, 1) * (manufactureInfo.bendingLineLength || 0) * ultimateTensileMaterial * manufactureInfo.bendingCoeffecient) / 9810;
    ...
    if (manufactureInfo.isinspectionCostDirty && manufactureInfo.inspectionCost !== null) {
      ...
      let inspectionCost = this.shareService.isValidNumber(
        ((Number(manufactureInfo.qaOfInspectorRate) / 60) * Number(manufactureInfo.inspectionTime) * Math.ceil(Number((manufactureInfo.samplingRate || 0) / 100) * Number(manufactureInfo.lotSize))) /
    ...
    if (manufactureInfo.isyieldCostDirty && manufactureInfo.yieldCost != null) {
      ...
      let yieldCost = this.shareService.isValidNumber(
        (1 - Number(manufactureInfo.yieldPer / 100)) * (Number(manufactureInfo.materialInfo?.totalCost || 0) + sum) -
        (1 - Number(manufactureInfo.yieldPer / 100)) * ((Number(manufactureInfo.materialInfo?.weight || 0) * Number(manufactureInfo.materialInfo?.scrapPrice || 0)) / 1000)
      );
  ...
  public calculationForSoftBending(manufactureInfo: ProcessInfoDto, fieldColorsList: any, manufacturingObj: ProcessInfoDto): ProcessInfoDto {
    ...
    let selected = sortedMachines.find((m) => m.bedLength === (manufactureInfo.bendingLineLength || 0));
    if (!selected) {
      selected = sortedMachines.find((m) => m.bedLength > (manufactureInfo.bendingLineLength || 0));
    }
    ...
    manufactureInfo.requiredCurrent = this.shareService.isValidNumber((manufactureInfo.bendingLineLength || 0) * 1.2);
  ...
  public calculationForstampingProgressive(manufactureInfo: ProcessInfoDto, fieldColorsList: any, manufacturingObj: ProcessInfoDto): ProcessInfoDto {
    ...
    for (let i = 0; i < manufactureInfo.subProcessFormArray?.controls?.length; i++) {
      ...
      if ([StampingType.BlankingPunching, StampingType.Piercing, StampingType.Compound].includes(Number(info.value.subProcessTypeID))) {
        ...
        const handlingInfo = manufactureInfo.handlingTimeList?.filter((x) => x.weight > (manufactureInfo.materialInfo?.weight || 0)).sort((s) => s.weight);
        ...
        blankingInfo.yieldCost = this.shareService.isValidNumber(
          (1 - Number(blankingInfo.yieldPer / 100)) * (Number(blankingInfo.materialInfo?.totalCost || 0) + sum) -
          (1 - Number(blankingInfo.yieldPer / 100)) * ((Number(blankingInfo.materialInfo?.weight || 0) * Number(blankingInfo.materialInfo?.scrapPrice || 0)) / 1000)
        );
  ...
  public calculationForTransferPress(manufactureInfo: ProcessInfoDto, fieldColorsList: any, manufacturingObj: ProcessInfoDto): ProcessInfoDto {
    ...
    maxDieLength = (manufactureInfo.materialInfoList?.[0]?.dimUnfoldedX || 0) * noOfStages + 120 * (noOfStages + 1);
    maxDieWidth = (manufactureInfo.materialInfoList?.[0]?.dimUnfoldedY || 0) + 200;
    maxDieHeight = (manufactureInfo.materialInfoList?.[0]?.dimUnfoldedZ || 0) * 2 + 100;
  ...
  public calculationForstampingStage(manufactureInfo: ProcessInfoDto, fieldColorsList: any, manufacturingObj: ProcessInfoDto, currentPart?: any): ProcessInfoDto {
    ...
    manufactureInfo.wallAverageThickness = (manufactureInfo.materialInfoList?.length ?? 0) > 0 ? (manufactureInfo.materialInfoList?.[0]?.wallAverageThickness ?? 0) : 0;
    ...
    manufactureInfo.projArea = (manufactureInfo.materialInfoList?.length ?? 0) > 0 ? (manufactureInfo.materialInfoList?.[0]?.runnerProjectedArea ?? 0) : 0;
  ...
  public calculationForForming(manufactureInfo: ProcessInfoDto, fieldColorsList: any, manufacturingObj: ProcessInfoDto): ProcessInfoDto {
    ...
    const handlingInfo = manufactureInfo.handlingTimeList?.filter((x) => x.weight > (manufactureInfo.materialInfo?.weight || 0) && x.isStageTooling == false).sort((s) => s.weight);
  ...
  public calculationForTubeLaser(manufactureInfo: ProcessInfoDto, fieldColorsList: any, manufacturingObj: ProcessInfoDto): ProcessInfoDto {
    ...
    manufactureInfo.requiredCurrent = manufactureInfo.materialInfoList?.[0]?.coilLength;
    manufactureInfo.requiredWeldingVoltage = manufactureInfo.materialInfoList?.[0]?.coilWidth;
    manufactureInfo.recBedSize = Math.round(Number(manufactureInfo.materialInfoList?.[0]?.coilLength || 0)) + ' x ' + Math.round(Number(manufactureInfo.materialInfoList?.[0]?.coilWidth || 0));
  ...
  public calculationForTubeBendingMetal(manufactureInfo: ProcessInfoDto, fieldColorsList: any, manufacturingObj: ProcessInfoDto): ProcessInfoDto {
    ...
    const materialInfo = (manufactureInfo?.materialInfoList?.length || 0) > 0 ? manufactureInfo?.materialInfoList[0] : null;
    ...
    if (manufactureInfo.isinjectionTimeDirty && manufactureInfo.injectionTime !== null) {
      ...
      let profileBendTime =
        ((materialInfo?.moldBoxLength || 0) <= 15
          ? bendLength / bendingSpeedDegSec / 2
          : (materialInfo?.moldBoxLength || 0) <= 30
            ? bendLength / bendingSpeedDegSec / 2
            : (materialInfo?.moldBoxLength || 0) <= 50
              ? bendLength / bendingSpeedDegSec / 2
              : (materialInfo?.moldBoxLength || 0) <= 100
                ? bendLength / bendingSpeedDegSec / 2
                : (materialInfo?.moldBoxLength || 0) <= 150
                  ? bendLength / bendingSpeedDegSec / 2
                  : 0) / 0.2;
  ...
  public calculationForShearing(manufactureInfo: ProcessInfoDto, fieldColorsList: any, manufacturingObj: ProcessInfoDto): ProcessInfoDto {
    ...
    if (manufactureInfo.isinspectionCostDirty && hasValue(manufactureInfo.inspectionCost)) {
      ...
      let inspectionCost = this.shareService.isValidNumber(
        ((Number(manufactureInfo.qaOfInspectorRate) / 60) * Number(manufactureInfo.inspectionTime) * Math.round(Number((manufactureInfo.samplingRate || 0) / 100) * Number(manufactureInfo.lotSize))) /
        Number(manufactureInfo.lotSize)
      );
