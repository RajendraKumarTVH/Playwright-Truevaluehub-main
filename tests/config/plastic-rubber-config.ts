import { MaterialInfoDto, MedbMachinesMasterDto, PartInfoDto } from 'src/app/shared/models';
import { SharedService } from 'src/app/modules/costing/services/shared.service';
import { MessagingService } from 'src/app/messaging/messaging.service';
import { PrimaryProcessType } from '../utils/constants';
...
export class PlasticRubberConfigService {
  ...
  getHeatTime(id: number, thickness: number) {
    ...
    const found = data.find((x) => x.materialFamilyId === id);
    return (found?.heatTime as any)?.[thickness] || 30;
  }
  ...
  thermoForminglookUpData = [
    { rawMaterial: 'PS', clampFactor: 1.2, desiredFormingTempF: 165, densityKg: 1050, specificHeatLb: 0.35, thermalConductivity: 0.12 },
    { rawMaterial: 'ABS', clampFactor: 1.5, desiredFormingTempF: 170, densityKg: 1040, specificHeatLb: 0.38, thermalConductivity: 1.2 },
    { rawMaterial: 'PP', clampFactor: 1.8, desiredFormingTempF: 180, densityKg: 910, specificHeatLb: 0.45, thermalConductivity: 0.15 },
    { rawMaterial: 'PE', clampFactor: 1.1, desiredFormingTempF: 165, densityKg: 950, specificHeatLb: 0.55, thermalConductivity: 0.2 },
    { rawMaterial: 'PET', clampFactor: 1.3, desiredFormingTempF: 160, densityKg: 1380, specificHeatLb: 0.3, thermalConductivity: 0.15 },
    { rawMaterial: 'PVC', clampFactor: 1.4, desiredFormingTempF: 135, densityKg: 1350, specificHeatLb: 0.25, thermalConductivity: 0.14 },
    { rawMaterial: 'PC', clampFactor: 2.0, desiredFormingTempF: 210, densityKg: 1200, specificHeatLb: 0.28, thermalConductivity: 0.2 },
  ];

  formingTimeList = [
    { rawMaterial: 'PS', thickness: 1, formingTime: 10 },
    { rawMaterial: 'ABS', thickness: 1, formingTime: 15 },
    { rawMaterial: 'PP', thickness: 1, formingTime: 20 },
    { rawMaterial: 'PE', thickness: 1, formingTime: 18 },
  ];
  ...
  getRubberMoldingCuringInfo(rubberType: string, thicknessMm: number) {
    ...
    const baseTimeMin = row.times[bucket as keyof (typeof this.rubberMoldingCuringData)[number]['times']];
