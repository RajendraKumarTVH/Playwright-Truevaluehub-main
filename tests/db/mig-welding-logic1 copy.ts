    ...
    async verifyWeldCycleTimeDetails(testData: any): Promise<void> {
        ...
        try {
            ...
            for (let idx = 0; idx < 2; idx++) {
                ...
                const uiStops = await this.page.getInputValueAsNumber(stopsLocator) || 0
            ...
            if (await this.page.ArcOnTime?.isVisible()) {
                const uiArcOn = await this.page.getInputValueAsNumber(this.page.ArcOnTime)
                expect.soft(uiArcOn).toBeCloseTo(breakdown.arcOnTime, 2)
                logger.info(`     UI Verification: ${uiArcOn.toFixed(4)} ≈ ${breakdown.arcOnTime.toFixed(4)}`)
            }

            // --- Arc Off Time ---
            logger.info(`   ✓ Arc Off Time: ${breakdown.arcOffTime.toFixed(4)} sec`)
            if (await this.page.ArcOffTime?.isVisible()) {
                const uiArcOff = await this.page.getInputValueAsNumber(this.page.ArcOffTime)
                expect.soft(uiArcOff).toBeCloseTo(breakdown.arcOffTime, 2)
                logger.info(`     UI Verification: ${uiArcOff.toFixed(4)} ≈ ${breakdown.arcOffTime.toFixed(4)}`)
            }

            // --- Total Weld Cycle Time (Dry Cycle Time) ---
            logger.info(`   ✓ Total Weld Cycle Time (Dry): ${breakdown.totalWeldCycleTime.toFixed(4)} sec`)
            if (await this.page.DryCycleTime?.isVisible()) {
                const uiDryCycle = await this.page.getInputValueAsNumber(this.page.DryCycleTime)
                expect.soft(uiDryCycle).toBeCloseTo(breakdown.totalWeldCycleTime, 2)
                logger.info(`     UI Verification: ${uiDryCycle.toFixed(4)} ≈ ${breakdown.totalWeldCycleTime.toFixed(4)}`)
            }
