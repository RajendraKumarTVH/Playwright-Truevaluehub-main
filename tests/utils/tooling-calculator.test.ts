...
import { ToolingCalculator, ToolingMaterialIM, CostToolingDto, ToolingMaterialInfoDto } from './tooling-calculator';

test.describe('Tooling Calculator - Injection Moulding', () => {
    const calculator = new ToolingCalculator();

    const sampleTool: CostToolingDto = {
        noOfCavity: 4,
        noOfTool: 1,
        noOfNewTool: 1,
    ...
    const materialInfos: ToolingMaterialInfoDto[] = [
        {
    ...
    test('should calculate Cavity Insert dimensions correctly', () => {
        const matInfo = { ...materialInfos[0] };
        ...
        expect(result.moldBaseLength).toBe(70);
        expect(result.moldBaseWidth).toBe(60);
        expect(result.moldBaseHeight).toBe(70);
        expect(result.quantity).toBe(4);
    });

    test('should calculate Core Insert dimensions correctly based on Cavity Insert', () => {
        // First calc Cavity Insert
        const cavMat = calculator.calculateMaterialCost({ ...materialInfos[0] }, sampleTool, materialInfos);
        const currentMatInfos = [cavMat, materialInfos[1]];

        const coreMat = { ...materialInfos[1] };
        ...
        expect(result.moldBaseLength).toBe(70);
        expect(result.moldBaseWidth).toBe(60);
        expect(result.moldBaseHeight).toBe(90);
    });

    test('should calculate Net Weight and Material Cost correctly', () => {
        ...
        expect(matInfo.netWeight).toBeCloseTo(1.57, 2);
        expect(matInfo.totalPlateWeight).toBeCloseTo(1.727, 3);
        expect(matInfo.totalRawMaterialCost).toBeCloseTo(8.635, 3);
    });
});
