...
export class WeldingCalculator {
	...
	public calculationForWelding(
		...
		if (
			...
			if (hasFormArray) {
				...
				// Fallback: if subProcessTypeInfos already present (populated elsewhere), sum their recommendTonnage
				if (
					manufactureInfo.subProcessTypeInfos &&
					manufactureInfo.subProcessTypeInfos.length > 0
				) {
					for (let i = 0; i < manufactureInfo.subProcessTypeInfos.length; i++) {
						const info: any = manufactureInfo.subProcessTypeInfos[i]
						totalWeldCycleTime += Number(info?.recommendTonnage || 0)
					}
					logger.info(
						`Fallback totalWeldCycleTime from subProcessTypeInfos: ${totalWeldCycleTime}`
					)
				}
			}
	...
	public weldingCommonCalc(
		...
		if (this.weldingMode !== 'seamWelding') {
			...
			manufactureInfo.powerConsumption = this.resolveField<number>(
				'powerConsumption',
				manufactureInfo.powerConsumptionKW,
			...
			logger.info(
				`Total Manufacturing Power Cost: curCycleTime: ${curCycleTime} powerConsumption: ${manufactureInfo.powerConsumptionKW} electricityUnitCost: ${manufactureInfo.electricityUnitCost}= totalPowerCost: ${manufactureInfo.totalPowerCost}`
			)
		}
	...
	public calculationsForWeldingPreparation(
		...
		if (manufactureInfo.iscycleTimeDirty && manufactureInfo.cycleTime != null) {
			...
			logger.info(
				`Cycle Time: handlingTime: ${handlingTime} weldingLength: ${weldingLength} noOfPasses: ${noOfPasses} feedPerREvRough: ${feedPerREvRough} deburringRPM: ${deburringRPM} -> CycleTime: ${cycleTime}`
			)
			if (
				Number(manufactureInfo?.processTypeID) ===
				ProcessType.WeldingPreparation
			...
			logger.info(
				`Cycle Time after Final Pass: feedPerREvFinal: ${feedPerREvFinal} -> CycleTime: ${cycleTime}`
			)
			manufactureInfo.cycleTime = this.resolveField<number>(
				'cycleTime',
				manufactureInfo.cycleTime,
		...
		if (
			...
			logger.info(
				`Direct Machine Cost: machineHourRate: ${manufactureInfo.machineHourRate} cycleTime: ${manufactureInfo.cycleTime} efficiency: ${efficiency} -> DirectMachineCost: ${directMachineCost}`
			)
		...
		if (
			...
			logger.info(
				`Direct Set Up Cost: noOfLowSkilledLabours: ${manufactureInfo.noOfLowSkilledLabours} setUpTime: ${manufactureInfo.setUpTime} lowSkilledLaborRatePerHour: ${manufactureInfo.lowSkilledLaborRatePerHour} noOfSkilledLabours: ${manufactureInfo.noOfSkilledLabours} skilledLaborRatePerHour: ${manufactureInfo.skilledLaborRatePerHour} efficiency: ${efficiency} lotSize: ${manufactureInfo.lotSize} -> DirectSetUpCost: ${directSetUpCost}`
			)
		}
		...
		if (
			...
			logger.info(
				`Direct Labor Cost: noOfLowSkilledLabours: ${manufactureInfo.noOfLowSkilledLabours} lowSkilledLaborRatePerHour: ${manufactureInfo.lowSkilledLaborRatePerHour} cycleTime: ${manufactureInfo.cycleTime} efficiency: ${efficiency} -> DirectLaborCost: ${directLaborCost}`
			)
		...
		if (
			...
			logger.info(
				`Inspection Cost: inspectionTime: ${manufactureInfo.inspectionTime} qaOfInspector: ${manufactureInfo.qaOfInspector} qaOfInspectorRate: ${manufactureInfo.qaOfInspectorRate} efficiency: ${efficiency} lotSize: ${manufactureInfo.lotSize} -> InspectionCost: ${inspectionCost}`
			)
			manufactureInfo.inspectionCost = this.resolveField<number>(
				'inspectionCost',
				manufactureInfo.inspectionCost,
	...
	public calculationsForWeldingCleaning(
		...
		// No. of Weld Passes and No. of Intermediate Start/Stops
		// noOfPasses => number of tool passes required (derived from element size and brush dia)
		// noOfIntermediateStartAndStop => number of start/stops computed from material details

		// Respect user override for weld passes
		if (
			manufactureInfo.isNoOfWeldPassesDirty &&
			manufactureInfo.noOfWeldPasses !== null
		) {
			manufactureInfo.noOfWeldPasses = Number(manufactureInfo.noOfWeldPasses)
			logger.info(
				`No of Weld Passes (override): ${manufactureInfo.noOfWeldPasses}`
			)
		} else {
			// default to computed passes
			manufactureInfo.noOfWeldPasses =
		...
		// No. of Intermediate Start/Stops - respect override if provided
		if (
			manufactureInfo.isnoOfIntermediateStartAndStopDirty &&
			manufactureInfo.noOfIntermediateStartAndStop !== null
		) {
			manufactureInfo.noOfIntermediateStartAndStop = Number(
				manufactureInfo.noOfIntermediateStartAndStop
			)
			logger.info(
				`No of Intermediate Start/Stops (override): ${manufactureInfo.noOfIntermediateStartAndStop}`
			)
		} else {
			...
			noOfIntermediateStartStops = weldingMaterialDetails.reduce(
				(sum: number, weldDetail: any) =>
					...
					(weldDetail.coreArea === 1
						? Number(weldDetail.coreVolume || 0)
						: Number(weldDetail.coreVolume || 0) *
						Number(weldDetail.coreArea || 1)),
				0
			)
			logger.info(
				`No of Intermediate Start/Stops (computed): ${noOfIntermediateStartStops}`
			)

			if (manufactureInfo.noOfIntermediateStartAndStop !== null) {
			...
			logger.info(
				`No of Intermediate Start/Stops: ${manufactureInfo.noOfIntermediateStartAndStop}`
			)
		}
		...
		logger.info(
			`Part Handling Time: reorientaionTime: ${reorientaionTime}, noOfWeldPasses: ${manufactureInfo.noOfWeldPasses} ${partHandlingTime}`
		)
	...
	private getMaxNearestWeightLoss(
		materialType: string,
		wireDiameter: number
	): number {
