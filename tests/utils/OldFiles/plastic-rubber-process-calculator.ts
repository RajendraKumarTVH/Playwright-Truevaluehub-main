...
import { MachineType, PartComplexity, PrimaryProcessType, ProcessType } from './constants';
import { PlasticRubberConfig } from './plastic-rubber-config';
import { CostingConfig } from '../config/costingConfig';
import { SharedService } from 'src/app/modules/costing/services/shared.service';
import { CastingConfigService } from '../config/casting-config';
...
export class ManufacturingCastingCalculatorService {
    ...
    public doCostCalculationForCasting(manufactureInfo: ProcessInfoDto, fieldColorsList: any, manufacturingObj: ProcessInfoDto): ProcessInfoDto {
        ...
        const matMetal = (IsHPDCCasting ? materialInfoList[0] : materialInfoList.filter((rec: any) => rec.secondaryProcessId === 1)[0]) || null;
        const matCore = materialInfoList.filter((rec: any) => rec.secondaryProcessId === 2)[0] || null;
        const matMold = materialInfoList.filter((rec: any) => rec.secondaryProcessId === 3)[0] || null;

        let matSlurryCost = null;
        let matPatternWax = null;
        if (IsInvestmentCasting) {
            matSlurryCost = materialInfoList.filter((rec: any) => rec.secondaryProcessId === 5)[0] || null;
            matPatternWax = materialInfoList.filter((rec: any) => rec.secondaryProcessId === 4)[0] || null;
        }
        ...
        const noOfCores = matCore?.coreCostDetails.length > 0 ? matCore?.coreCostDetails.reduce((accVal: any, currentRec: any) => accVal + currentRec.noOfCore, 0) || 1 : 0;
        if (IsHPDCCasting) {
            processHPDC = manufactureInfo?.processInfoList.filter((rec: any) => rec.processTypeID === ProcessType.HighPressureDieCasting)[0] || null;
        } else if (IsLPDCCasting) {
            processLPDC = manufactureInfo?.processInfoList.filter((rec: any) => rec.processTypeID === ProcessType.LowPressureDieCasting)[0] || null;
            processCoreAssembly = manufactureInfo?.processInfoList.filter((rec: any) => rec.processTypeID === ProcessType.CastingCoreAssembly)[0] || null;
            ...
            processGDC = manufactureInfo?.processInfoList.filter((rec: any) => rec.processTypeID === ProcessType.GravityDieCasting)[0] || null;
        } else if (IsInvestmentCasting) {
            ;
        }
        if (IsNoBakeCasting) {
            processMoldMaking = manufactureInfo?.processInfoList.filter((rec: any) => rec.processTypeID === ProcessType.CastingMoldMaking)[0] || null;
        } else {
            processMoldMaking = manufactureInfo?.processInfoList.filter((rec: any) => rec.processTypeID === ProcessType.MoldPerparation)[0] || null;
        }
        processMelting = manufactureInfo?.processInfoList.filter((rec: any) => rec.processTypeID === ProcessType.MeltingCasting)[0] || null;
        ...
        if (Number(manufactureInfo?.processTypeID) === ProcessType.CastingCorePreparation) {
            ...
            if (IsNoBakeCasting || IsGreenCasting || IsGDCCasting || IsLPDCCasting) {
                const selectedMatcore =
                    materialInfoList.filter((rec: any) => rec.secondaryProcessId === 2).map((x: any) => x.coreCostDetails.find((core: any) => core.coreCostDetailsId === manufactureInfo.subProcessTypeID)) || [];
    ...
    public calculationForPouringCasting(manufactureInfo: ProcessInfoDto, fieldColorsList: any, manufacturingObj: ProcessInfoDto, laborRateDto: LaborRateMasterDto[]): ProcessInfoDto {
        ...
        manufactureInfo.totalFactorySpaceRequired = this.shareService.isValidNumber((manufactureInfo?.machineMaster?.maxLength * manufactureInfo?.machineMaster?.maxWidth) / 1000000);
        if (manufactureInfo.countryList && manufactureInfo.mfrCountryId) {
            const country = manufactureInfo.countryList.find((x: any) => x.countryId == manufactureInfo.mfrCountryId);
            if (country) {
