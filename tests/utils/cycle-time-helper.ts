import { CycleTimeBreakdown } from "./interfaces"

export function calculateCycleTimeBreakdown(
    calculated: any,
    efficiency: number
): CycleTimeBreakdown {
    const efficiencyDecimal = efficiency > 1 ? efficiency / 100 : efficiency

    const subProcessCycleTime = calculated.subProcessTypeInfos?.reduce(
        (sum: number, info: any) => sum + (info.recommendTonnage || 0),
        0
    ) || 0
...
export function logCycleTimeBreakdown(breakdown: CycleTimeBreakdown, logger: any): void {
